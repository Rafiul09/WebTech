...
function lv_search2_doc($query, $category, $slot)
{
    ...
    if (!empty($query) && empty($category) && empty($slot)) {
        ...
        $qury = "SELECT * FROM doctors WHERE ";
        $conditions = array();
        ...
        $qury .= implode(' OR ', $conditions);
    } elseif (!empty($query) && !empty($category) && !empty($slot)) {
        $qury = "SELECT * FROM doctors WHERE (D_firstName LIKE '%$search%' OR D_lastName LIKE '%$search%') AND D_specialist LIKE '%$category%' AND ";
        ...
        $qury = "SELECT * FROM doctors WHERE (D_firstName LIKE '%$search%' OR D_lastName LIKE '%$search%') AND ";
        ...
        $qury = "SELECT * FROM doctors WHERE D_specialist = '$category' AND ";
        $conditions = array();
...
function insertpatient($patient_id, $name, $gen, $dob, $em, $phon, $address, $pass)
{
    $conn = get_conn();
    $sql_Rin = "INSERT INTO Patient (P_ID, P_Name, P_Gender, P_DOB, P_Email, P_Phone, P_Address) VALUES ('$patient_id', '$name', '$gen', '$dob', '$em', '$phon', '$address')";
